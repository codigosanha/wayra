...
public class ClsVentaPendiente {
    ...
    public void agregarVenta(ClsEntidadVenta Venta) {
        try {
            ...
            JOptionPane.showMessageDialog(null, "¡Venta Pendiente Guardada!", "Mensaje del Sistema", 1);
    ...
    public void modificarVenta(String codigo, ClsEntidadVenta Venta) {
        try {
            CallableStatement statement = connection.prepareCall("{call SP_U_VentaPendiente(?,?,?,?,?,?,?,?,?,?,?,?,?)}");
    ...
    public void updateEstado(String codigo, ClsEntidadVenta Venta) {
        try {
    ...
    public ResultSet listarVentaPorParametro(String criterio, String busqueda) throws Exception {
        ResultSet rs = null;
        try {
            CallableStatement statement = connection.prepareCall("{call SP_S_VentaPendientePorParametro(?,?)}");
    ...
    public ResultSet obtenerUltimoIdVenta() throws Exception {
        ResultSet rs = null;
        try {
            CallableStatement statement = connection.prepareCall("{call SP_S_UltimoIdVentaPendiente()}");
            rs = statement.executeQuery();
            return rs;
        } catch (SQLException SQLex) {
            throw SQLex;
    ...
    public void actualizarVentaEstado(String codigo, ClsEntidadVenta Venta) {
        try {
            CallableStatement statement = connection.prepareCall("{call SP_U_EstadoVentaPendiente(?,?)}");
            statement.setString("pidventa", codigo);
            statement.setString("pestado", Venta.getStrEstadoVenta());
            statement.executeUpdate();
